...
for i=1:cls
    ...
    for j=1:num
        ...
        if p(1)>0 && p(1)<482 && p(2)>0 && p(2)<322
            handles.pixs{i}(j) = [handles.im(p(1),p(2),1) handles.im(p(1),p(2),2) handles.im(p(1),p(2),3) p(1)/481 p(2)/481];
        end
    end
end
...
for y=1:row
    for x=1:col
        points(id,:) = [handles.im(y,x,1) handles.im(y,x,2) handles.im(y,x,3) x/ y];
        id = id + 1;
    end
end
